...
DELETE V FROM VendorUser AS V INNER JOIN Employee ON Username = EmployeeID
WHERE Username = ? AND EmployeeLevel = 'CustomerRep';

-- Obtain a sales report for a particular month
SELECT ItemName, CurrentHighestBidPrice AS SoldPrice
...
SELECT CustomerID AS Buyer, A.AuctionID, Seller, BidPrice, CurrentHighestBidPrice AS SoldPrice, BidTime
...
-- Record a sale
UPDATE Auction SET SaleStatus = 'Paid' WHERE AuctionID = ?;
...
SELECT A.AuctionID, ItemID, NumCopies, CustomerID AS WinningBidder, BidTime, BidPrice, CurrentHighestBidPrice AS SoldPrice, EndDate
FROM Auction AS A INNER JOIN Bid AS B ON A.WinningBidID = B.BidID
...
FROM Item INNER JOIN 
WHERE ItemType IN ('DVD') AND MATCH (ItemName) AGAINST ('titanic movie' IN NATURAL LANGUAGE MODE WITH QUERY EXPANSION);
...
-- Get all auctions
SELECT AuctionID, MinBidPrice, CurrentHighestBidPrice, NumCopies, Seller, ItemID, OpenDate, EndDate FROM Auction;

-- Get auctions by CustomerID
SELECT AuctionID, MinBidPrice, CurrentHighestBidPrice, NumCopies, Seller, ItemID, OpenDate, EndDate
FROM Auction AS A
...
-- Get auction data
SELECT A.AuctionID, A.ItemID, ItemType, ItemName, Description, BidIncrement, MinBidPrice, CurrentHighestBidPrice, CustomerID, FirstName, LastName
...
-- Show all items
SELECT ItemID, ItemName, ItemType, Description, Quantity FROM Item;

-- Show bid history for a particular auction
SELECT AuctionID, CustomerID, BidTime, BidPrice
FROM Bid
WHERE AuctionID = ?
ORDER BY BidTime DESC;

-- History of all current and past auctions a customer has taken part in
SELECT AuctionID, CustomerID, MAX(BidTime) AS LastBidTime, MAX(BidPrice) AS LastBidPrice
...
-- Search customers
SELECT CustomerID, FirstName, LastName, Address, City, State, ZipCode, Telephone, Email, CreditCardNum, Rating
FROM Customer INNER JOIN VendorUser ON Username = CustomerID
WHERE FirstName LIKE CONCAT('%', ?, '%') OR LastName LIKE CONCAT('%', ?, '%');

-- Get customer
SELECT CustomerID, UserPassword, FirstName, LastName, Address, City, State, ZipCode, Telephone, Email, CreditCardNum, Rating
FROM Customer INNER JOIN VendorUser ON Username = CustomerID
WHERE CustomerID = ?;

-- Get employees
SELECT EmployeeID, FirstName, LastName, Address, City, State, ZipCode, Email, StartDate, SSN, HourlyRate
FROM VendorUser INNER JOIN Employee ON Username = EmployeeID
WHERE EmployeeLevel = 'CustomerRep';

-- Get employee
SELECT EmployeeID, UserPassword, FirstName, LastName, Address, City, State, ZipCode, Telephone, Email, SSN, HourlyRate
